...
alpha_grid <- c(0.25, 0.5, 0.75) #weights on geographic proximity (try a relatively sparse grid of them)
...
minimum_no_cluster_factor_grid <- c(5, 10, 15, 20, 25) #grid to check what targeted maximum cluster size should be (in block groups)
...
#if (file.exists(paste0("DataV2/CoreLogic/output/", filename_to_output)) == FALSE) { #remove this condition if you want to overwrite existing files
...
for (j in unique(US_BLOCK$CBSA)) {
  ...
  for (i in unique(US_BLOCK_MSA[[municipalitydef]])) {
     ...
     for (zone in unique(US_BLOCK_MP$Assigned_zoningcode)) {
       ...
       if (zone == "") { #if zone missing, cluster 
...
for (minimum_no_cluster_factor in minimum_no_cluster_factor_grid) { #Writing data for each minimum_no_cluster_factor within municipality definition and alpha_grid definition
  ...
  #Checking if clusters line up closely with zoning district definitions
