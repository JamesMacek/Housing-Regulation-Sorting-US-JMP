...
CleanLotSize <- 2 #Parameter to remove minimum lot size observations if minimum lot size is 
                  #`x' times the average of the average lot size of regulated structures.
...
Main_regulation_index <- which(grepl(paste0("alpha_", Main_alpha), e.filelist) == TRUE & 
                               grepl(paste0("minClust", Main_minClust), e.filelist) == TRUE &
                               grepl(Main_municipality, e.filelist) == TRUE) 

Main_regulation <- Regulation[[Main_regulation_index]]

Main_regulation <- Main_regulation %>% select(State, County, Tract, BlockGroup, Assigned_municipality, CBSA,
                                    Assigned_geocoded_municipality, ZoningDistrictID, UnitDensityRestriction, LandValueDensity, 
                                    IncomeStringency, IncomeStringency_preCovid, AverageLotSize_minimum_struct)
...
#setting some observations to NA that have the measured mode above 25% of the average lot size 
US_BLOCK["IncomeStringency_cl"] <- US_BLOCK$IncomeStringency
US_BLOCK$IncomeStringency_cl[US_BLOCK$UnitDensityRestriction > CleanLotSize*US_BLOCK$AverageLotSize_minimum_struct] <- NA
US_BLOCK["IncomeStringency_preCovid_cl"] <- US_BLOCK$IncomeStringency_preCovid
US_BLOCK$IncomeStringency_preCovid_cl[US_BLOCK$UnitDensityRestriction > CleanLotSize*US_BLOCK$AverageLotSize_minimum_struct] <- NA
US_BLOCK["UnitDensityRestriction_cl"] <- US_BLOCK$UnitDensityRestriction
US_BLOCK$UnitDensityRestriction_cl[US_BLOCK$UnitDensityRestriction > CleanLotSize*US_BLOCK$AverageLotSize_minimum_struct] <- NA

#NOTE: IT DOES NOT MATTER FOR THE FACTS WHICH CENSORING THRESHOLD YOU CHOOSE ABOVE. CleanLotSize is fine but it can be anything.

#Demeaning by CBSA for each type
...
US_BLOCK <- US_BLOCK %>% st_drop_geometry() %>% dplyr::select(-STATE.x, -COUNTY.x) 
...
Regulation_data <- US_BLOCK %>% dplyr::select(State, County, Tract, BlockGroup, CBSA, UnitDensityRestriction, IncomeStringency, LandValueDensity) 
...
for (file in e.filelist) {
  ...
  Regulation.file <- Regulation[[index]] %>% select(State, County, Tract, BlockGroup, CBSA, IncomeStringency, 
                                                    UnitDensityRestriction, AverageLotSize_minimum_struct, 
  ...
  Regulation.file$IncomeStringency_cl[Regulation.file$UnitDensityRestriction > CleanLotSize*Regulation.file$AverageLotSize_minimum_struct] <- NA
  Regulation.file["UnitDensityRestriction_cl"] <- Regulation.file$UnitDensityRestriction
  Regulation.file$UnitDensityRestriction_cl[Regulation.file$UnitDensityRestriction > CleanLotSize*Regulation.file$AverageLotSize_minimum_struct] <- NA
  ...
  names(Regulation.file)[names(Regulation.file) == 'UnitDensityRestriction_cl'] <- paste0("uDR", 
                                                                                    sub("Regulation_measurements_", 
                                                                                        "", file))
  #Labeling modes for each type of structure
  ...
  Regulation.file <- Regulation.file %>% select(-IncomeStringency, -IncomeStringency_cl, -UnitDensityRestriction, -AverageLotSize_minimum_struct)
...
#Saving datasets for use with other programs.
