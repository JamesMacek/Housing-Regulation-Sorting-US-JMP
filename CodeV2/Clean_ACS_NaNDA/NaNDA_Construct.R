...
nanda_arts <- nanda_arts[nanda_arts$year == 2017 | nanda_arts$year == 2007,] #keeping latest date (2017) + 10 years long diff
nanda_arts <- nanda_arts %>% select(tract_fips10, aden_7111, year,
...
nanda_arts_hist <- nanda_arts[nanda_arts$year == 2007,] %>% rename(perf_arts_dens_hist = perf_arts_dens,
                                                                   spec_sports_dens_hist = spec_sports_dens,
                                                                   casino_dens_hist = casino_dens,
                                                                   rec_act_dens_hist = rec_act_dens) %>% select(-year)
nanda_arts <- nanda_arts[nanda_arts$year == 2017,] %>% select(-year)
nanda_arts <- left_join(nanda_arts, nanda_arts_hist, by = c("tract_fips10"))
rm(nanda_arts_hist)
...
vars_to_merge <- c("perf_arts_dens", "spec_sports_dens", 
                   ...
                   "perf_arts_dens_hist", "spec_sports_dens_hist", 
                   "casino_dens", "rec_act_dens_hist")
...
Block_geo_2020 <- Block_geo_2020 %>% mutate(State = as.numeric(State), #Converting to numeric
                                            County = as.numeric(County),
                                            Tract = as.numeric(Tract),
                                            BlockGroup = as.numeric(BlockGroup))
