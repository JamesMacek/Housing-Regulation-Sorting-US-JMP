...
#Function to load data without changing memory in object name
load_obj <- function(f)
{
  env <- new.env()
  nm <- load(f, env)[1]
...
#loading data
unmatched_LatLon_city <- load_obj(paste0("DataV2/US_Data/Output/", load_GoogleMaps_CBD)) %>% select(GoogleEarthLat, GoogleEarthLon)
US_CBSA_unmatched["GoogleEarthLat"] <- unmatched_LatLon_city$GoogleEarthLat
US_CBSA_unmatched["GoogleEarthLon"] <- unmatched_LatLon_city$GoogleEarthLon
