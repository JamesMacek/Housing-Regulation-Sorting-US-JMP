...
bysort State County Tract BlockGroup: egen Assigned_municipality = mode(municipalityname) if keepindicator == 1, minmode

bysort State County Tract BlockGroup: egen Assigned_zoningcode = mode(zoningcode) if keepindicator == 1, minmode

bysort State County Tract BlockGroup: egen Assigned_geocoded_municipality = mode(Geocoded_municipality) if keepindicator == 1, minmode
...
*Generating mode, median, p(25) and p(75) of lot size distributions for each structure type
...
forval i = 1/`structureListSize' {
	...
	bysort State County Tract BlockGroup: egen `curStructure'_statp75 = pctile(`curStructure'_lotsize), p(75)
