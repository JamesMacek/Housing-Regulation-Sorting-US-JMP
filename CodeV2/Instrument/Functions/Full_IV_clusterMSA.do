...
*Storing Pooled OLS estimates with baseline controls in local for Placebo.do
ivreghdfe log_Amenity log_Average_Income Average_slope_control Avg_Slope Outer_slope_control `baseline_controls' if !missing(instrument_Avg_Slope_spec_`Pref_spec'), absorb(i.eCBSA_NAME) cluster(eCBSA_NAME) partial(`baseline_controls') sfirst
local Omega_OLS _b[log_Average_Income]
di `Omega_OLS'
...
forvalues i = 1/7 { 
	...
	*STORING LOCAL OF ESTIMATES FOR PLACEBO.DO FILE
	local Omega_`i' _b[log_Average_Income]
	di `Omega_`i''
	
	*IF BASELINE, STORING THAT AS WELL
	if `Pref_spec' == `i' {
		local Omega_baseline _b[log_Average_Income]
		di `Omega_baseline'
	}
	
	outreg2 using "DataV2/US_Data/Instrument/FullIVMSAClusterRobust", `replace' tex(pretty) label nocon dec(4) nor2 ///
addtext(Specification, IV, Donut, `donutString', Base Controls, `baseControls_indicator', Amen/Topo Controls, `AmenTopo_Controls_indicator', Density Control, `DensityControl_indicator', KP-FStat, `KP', Cluster, MSA) 
	...
	*ROBUSTNESS: Remove Outer buffer to show that estimates significantly change
	...
	outreg2 using "DataV2/US_Data/Instrument/FullIVMSAClusterRobust_noMaxSlope", `replace' tex(pretty) label nocon dec(4) nor2 ///
addtext(Specification, IV, Donut, `donutString', Base Controls, `baseControls_indicator', Amen/Topo Controls, `AmenTopo_Controls_indicator', Density Control, `DensityControl_indicator', KP-FStat, `KP', Cluster, MSA) 
...
foreach t in l m h {
	...
	*Saving local for Placebo.do
	local Omega_`t' _b[log_Average_Income]
	di `Omega_`t''
	
	outreg2 using "DataV2/US_Data/Instrument/IVMSAClust_byIncomeType", `replace' tex(pretty) label nocon dec(4) nor2 ///
...
foreach t in l m h {
	*Check, NO MAX SLOPE
	...
	outreg2 using "DataV2/US_Data/Instrument/IVMSAClust_byIncomeType_noMaxSlope", `replace' tex(pretty) label nocon dec(4) nor2 ///
addtext(Specification, IV, Donut, `instrumentString', Base Controls, `baseControls_indicator', Amen/Topo Controls, `AmenTopo_Controls_indicator', Density Control, `DensityControl_indicator') drop(`baseline_controls')
local replace
}
...
foreach t in l m h {
	*OLS by income type
	...
	outreg2 using "DataV2/US_Data/Instrument/OLSMSAClust_byIncomeType", `replace' tex(pretty) label nocon dec(4) nor2 ///
...
*_______________________________________________________________________________
*Baseline specification IVPPML by income type (use control function approach with ppmlhdfe and reghdfe), no bootstrap 
...
foreach t in l m h {
	*Take first stage 
	ppmlhdfe WithinCity_Amenity_`t' l_Avg_Inc_hat Average_slope_control Avg_Slope Outer_slope_control `baseline_controls', absorb(i.eCBSA_NAME) cluster(eCBSA_NAME)
		...
		outreg2 using "DataV2/US_Data/Instrument/IV_PPML_MSAClust_byIncomeType", `replace' tex(pretty) label nocon dec(4) nor2 ///
...
foreach t in l m h {
	ppmlhdfe WithinCity_Amenity_`t' log_Average_Income Average_slope_control Avg_Slope Outer_slope_control `baseline_controls' if !missing(instrument_Avg_Slope_spec_`Pref_spec'), absorb(i.eCBSA_NAME) cluster(eCBSA_NAME)
		...
		outreg2 using "DataV2/US_Data/Instrument/PPML_MSAClust_byIncomeType", `replace' tex(pretty) label nocon dec(4) nor2 ///
