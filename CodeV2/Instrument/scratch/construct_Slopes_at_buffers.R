...
#Coercing land mass to numeric
for (buffer_dist in Buffers) {
 US_BLOCK[paste0("buffer_area_", buffer_dist, "m")] <- as.numeric(US_BLOCK[[paste0("buffer_area_", buffer_dist, "m")]])
}

#Creating specifications (each indexed by order in which they appear, see Instrument_Parameters.R)
index <- 1
for (buffer_dist in Exclusion_buffers_para) {

  #1. taking exclusion buffer, calculating what slopes would be in this exclusion buffer excluding slopes at own block group
    #First, need set of implied land mass weights for control = land mass of own block group / land mass of exclusion buffer
    buffer_weights <- US_BLOCK$buffer_area_0m/US_BLOCK[[paste0("buffer_area_", buffer_dist, "m")]] #always fractional
  
    US_BLOCK[paste0("control_Avg_Slope_spec_", index)] <- (US_BLOCK[[paste0("Avg_Slope_", buffer_dist, "m")]]/(1 - buffer_weights)) - 
                                                          (buffer_weights/(1-buffer_weights))*US_BLOCK$Avg_Slope_0m
  
  #2. taking Inclusion buffer, calculating what slopes would be in this exclusion buffer excluding slopes at own block group
    #First, need set of implied land mass weights for control = land mass of own block group / land mass of exclusion buffer
    buffer_weights = (US_BLOCK[[paste0("buffer_area_", buffer_dist, "m")]])/(US_BLOCK[[paste0("buffer_area_", Inclusion_buffers_para[which(Exclusion_buffers_para == buffer_dist)], "m")]])
    
    US_BLOCK[paste0("instrument_Avg_Slope_spec_", index)] <- (US_BLOCK[[paste0("Avg_Slope_", Inclusion_buffers_para[which(Exclusion_buffers_para == buffer_dist)], "m")]]/(1 - buffer_weights)) - 
                                                              (buffer_weights/(1-buffer_weights))*US_BLOCK[[paste0("Avg_Slope_", buffer_dist, "m")]]
...
US_BLOCK <- US_BLOCK %>% dplyr::select(State, County, Tract, BlockGroup, CBSA, CBSA_NAME,
                                       ...
                                       starts_with("Avg_Slope_"), starts_with("buffer_area"))

#Dropping geometry and saving as .dta file
US_BLOCK <- US_BLOCK %>% st_drop_geometry()
write_dta(US_BLOCK, "DataV2/US_Data/Instrument/Instrument_constructed_buffers.dta")