...
getLocationAmenity <- function(Master_data, bySkill, incomeType) {
  ...
  if (bySkill == FALSE) {
    ...
    Amenity <- Master_data[[paste0("exogenous_Amenity_", incomeType)]]*(Master_data$Avg_income^(Omega))
    return(Amenity)
  }
  ...
  if (bySkill == TRUE) {
    ...
    for (skill in c("College", "NoCollege")) {
      ...
      Amenity[[skill]] <- Master_data[[paste0("exogenous_Amenity_", skill, "_", incomeType)]]*(Master_data$Avg_income^(Omega))
