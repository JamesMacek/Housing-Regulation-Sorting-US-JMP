...
#########################################################################################################################
  ...
  ncores  <- min(nrow(totalGrid), detectCores() - 1)
  ...
  foreach(row = 1:(nrow(totalGrid)) ) %dopar% { 
    ...
    #Control new distribution as isoelastic function of old one. Rescale by relative means so F2 does not affect mean regulation, just its dispersion.
