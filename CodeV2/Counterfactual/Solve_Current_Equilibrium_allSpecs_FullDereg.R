...
ncores <- min(detectCores() - 1, 6) #six cores for now
...
#Solving for fundamental productivity (if bySkill == FALSE)
if (EquilibriumType$bySkill == FALSE) {
  
  Equilibrium_objects[paste0("fundamental_prod")] <- Equilibrium_objects[["PooledWage"]]/(Equilibrium_objects[["City_Population"]]^(Agglomeration_elast))
  
}else{ #If bySkill == TRUE...
  
  for (skill in skillVector) {
    name_of_skill <- skillName[which(skill == skillVector)]
    
    #creating fundamental productivity after accounting for agglomeration economies
    Equilibrium_objects[paste0("fundamental_prod_", skill)] <- Equilibrium_objects[paste0("Productivity_", skill)]/(Equilibrium_objects[["City_Population"]]^(Agglomeration_elast))
...
if (pref == "SG") {
  ...
  eq_error_tol_Mobility <- 10 #SG equilibria are a bit harder to solve, so increase equilibrium error to 10 households...
...
while (error_Mobility > eq_error_tol_Mobility | error_Spendshare > eq_error_tol_Spendshares) {
  ...
  #UPDATE ADJUSTMENT SPEED IF TAKING LONG
  if (iter == 500) {
    #cut adjustment speed in half if iterations are taking too long
    adjustment_speed_Mobility <- adjustment_speed_Mobility/2
  }
  
  if (iter == 1000) {
    #cut adjustment speed in half if iterations are taking too long
    adjustment_speed_Mobility <- adjustment_speed_Mobility/6
  }
  
  #PART 0.5: Updating productivity if needed
  if (EquilibriumType$EndogenousProductivity == TRUE) {
  
    Equilibrium_objects["pop_running_total"] <- rep(0, nrow(Equilibrium_objects)) #running total of population in neighborhood
    for (skill in skillVector) {
      name_of_skill <- skillName[which(skill == skillVector)]
      for (incomeType in 1:7) {
        Equilibrium_objects["pop_running_total"] <- Equilibrium_objects[["pop_running_total"]] + Equilibrium_objects[[paste0("Population_type_", name_of_skill, incomeType)]]
    ...
    }else{ #Else, if bySkill == TRUE
      for (skill in skillVector) {
        Equilibrium_objects[paste0("Productivity_", skill)] <- Equilibrium_objects[[paste0("fundamental_prod_", skill)]]*(Equilibrium_objects[["City_Population"]]^(Agglomeration_elast))
...
rm(list = str_replace(
          str_replace(
          str_replace(ls(), "EquilibriumType", "_"),
          "solver", "_"),
          "check_init_eq", "_")) #removing all objects we want out that are not in memory
