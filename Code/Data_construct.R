...
#2010 CBSA boundaries
US_CBSA_2010 <- st_read("gz_2010_us_310_m1_500k.shp")
US_CBSA_2010 <- US_CBSA_2010[US_CBSA_2010$LSAD == "Metro",] #Keeping Metropolitan statistical areas, MSAs. These are 2010 definitions. From US census bureau


#2010 census tracts
US_TRACT_2010 <- st_read("CensusTract2010/US_tract_2010.shp") #From IPUMS NHGIS website
...
CBDs <- CBDs %>% select(CBSA_code, CBSA_name, GoogleEarthLat, GoogleEarthLon)
names(CBDs)[names(CBDs) == 'CBSA_code'] <- 'CBSA'
#merging with 2010 definitions to see if these metros are the same
US_CBSA_2010$CBSA <- as.integer(US_CBSA_2010$CBSA)
US_CBSA_2010 <- full_join(US_CBSA_2010, CBDs, by = "CBSA") #looks good!
...
US_TRACT_2010_JOINED["log_income_to_value"] <-  log(US_TRACT_2010_JOINED$Average_income/US_TRACT_2010_JOINED$fmode.Med_house_val_tr_ACS_06_10) #Wage/House price
...
US_TRACT_2010_JOINED <- US_TRACT_2010_JOINED %>% group_by(CBSA) %>% mutate(rank_density_CBSA = order(order(Housing_density, decreasing = FALSE))/max(order(order(Housing_density, decreasing = FALSE))))#Have to run order twice for some reason for it to work. Dividing by max_order to make plot manageable
...
US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED[!is.na(US_TRACT_2010_JOINED$qyye001) & !is.na(US_TRACT_2010_JOINED$qyye002) & !is.na(US_TRACT_2010_JOINED$qyye003) & US_TRACT_2010_JOINED$qyye001 > 0 &
                                                 US_TRACT_2010_JOINED$qyye002 + US_TRACT_2010_JOINED$qyye003 > 0,] %>% st_drop_geometry()
US_TRACT_2010_JOINED_D["log_single_family_density"] <- log((US_TRACT_2010_JOINED_D$qyye002 + US_TRACT_2010_JOINED_D$qyye003)/US_TRACT_2010_JOINED_D$fsum.LAND_AREA)
US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED_D  %>% group_by(CBSA) %>% mutate(demeaned_log_single_family_density = log_single_family_density - mean(log_single_family_density))
US_TRACT_2010_JOINED <- left_join(US_TRACT_2010_JOINED, US_TRACT_2010_JOINED_D, by = c("State", "County", "Tract"), suffix = c("", ".y")) %>% select(-ends_with(".y"))
rm(US_TRACT_2010_JOINED_D)

US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED[!is.na(US_TRACT_2010_JOINED$qyye001) & !is.na(US_TRACT_2010_JOINED$qyye002) & !is.na(US_TRACT_2010_JOINED$qyye003) & 
                                                 US_TRACT_2010_JOINED$qyye001 > 0 & US_TRACT_2010_JOINED$qyye002 + US_TRACT_2010_JOINED$qyye003 < US_TRACT_2010_JOINED$qyye001,] %>% st_drop_geometry()
US_TRACT_2010_JOINED_D["log_multi_family_density"] <- log((US_TRACT_2010_JOINED_D$qyye001 - (US_TRACT_2010_JOINED_D$qyye002 + US_TRACT_2010_JOINED_D$qyye003))/US_TRACT_2010_JOINED_D$fsum.LAND_AREA)
US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED_D  %>% group_by(CBSA) %>% mutate(demeaned_log_multi_family_density = log_multi_family_density - mean(log_multi_family_density))
US_TRACT_2010_JOINED <- left_join(US_TRACT_2010_JOINED, US_TRACT_2010_JOINED_D, by = c("State", "County", "Tract"), suffix = c("", ".y")) %>% select(-ends_with(".y"))
rm(US_TRACT_2010_JOINED_D)

#20+ Building Density
US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED[!is.na(US_TRACT_2010_JOINED$qyye008) & !is.na(US_TRACT_2010_JOINED$qyye009) & US_TRACT_2010_JOINED$qyye008 + US_TRACT_2010_JOINED$qyye009 > 0,] %>% st_drop_geometry()
US_TRACT_2010_JOINED_D["log_20_building_density"] <- log((US_TRACT_2010_JOINED_D$qyye008 + US_TRACT_2010_JOINED_D$qyye009)/US_TRACT_2010_JOINED_D$fsum.LAND_AREA)
US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED_D  %>% group_by(CBSA) %>% mutate(demeaned_log_20_building_density = log_20_building_density - mean(log_20_building_density))
US_TRACT_2010_JOINED <- left_join(US_TRACT_2010_JOINED, US_TRACT_2010_JOINED_D, by = c("State", "County", "Tract"), suffix = c("", ".y")) %>% select(-ends_with(".y"))
rm(US_TRACT_2010_JOINED_D)

#All other densities (i.e. between 2 and 19 housing units)
US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED[!is.na(US_TRACT_2010_JOINED$qyye008) & !is.na(US_TRACT_2010_JOINED$qyye009) &
                                                 !is.na(US_TRACT_2010_JOINED$qyye002) & !is.na(US_TRACT_2010_JOINED$qyye003) & US_TRACT_2010_JOINED$qyye002 + US_TRACT_2010_JOINED$qyye003 + 
                                                 US_TRACT_2010_JOINED$qyye008 + US_TRACT_2010_JOINED$qyye009 < US_TRACT_2010_JOINED$qyye001,] %>% st_drop_geometry()
US_TRACT_2010_JOINED_D["log_middle_density"] <- log((US_TRACT_2010_JOINED_D$qyye001 - (US_TRACT_2010_JOINED_D$qyye002 + US_TRACT_2010_JOINED_D$qyye003 + 
                                                                                       US_TRACT_2010_JOINED_D$qyye008 + US_TRACT_2010_JOINED_D$qyye009))/US_TRACT_2010_JOINED_D$fsum.LAND_AREA)
US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED_D  %>% group_by(CBSA) %>% mutate(demeaned_log_middle_density = log_middle_density - mean(log_middle_density))
US_TRACT_2010_JOINED <- left_join(US_TRACT_2010_JOINED, US_TRACT_2010_JOINED_D, by = c("State", "County", "Tract"), suffix = c("", ".y")) %>% select(-ends_with(".y"))
rm(US_TRACT_2010_JOINED_D)

##################CORRECTION: Want to decompose each into 3 components (do NOT use log scale. This is about ordering!)_____________________
...
#Income subset
US_TRACT_2010_JOINED_INC <- US_TRACT_2010_JOINED[!(is.na(US_TRACT_2010_JOINED$Average_income)),] %>% st_drop_geometry()
...
#Price to income
US_TRACT_2010_JOINED_INC <- US_TRACT_2010_JOINED[!(is.na(US_TRACT_2010_JOINED$Average_income)) & !is.na(US_TRACT_2010_JOINED$fmode.Med_house_val_tr_ACS_06_10),] %>% st_drop_geometry()
US_TRACT_2010_JOINED_INC <- US_TRACT_2010_JOINED_INC %>% group_by(CBSA) %>% mutate(demeaned_log_Income_to_value = log(Average_income/fmode.Med_house_val_tr_ACS_06_10) - mean(log(Average_income/fmode.Med_house_val_tr_ACS_06_10)))
US_TRACT_2010_JOINED <- left_join(US_TRACT_2010_JOINED, US_TRACT_2010_JOINED_INC, by = c("State", "County", "Tract"), suffix = c("", ".y")) %>% select(-ends_with(".y"))
rm(US_TRACT_2010_JOINED_INC)
...
US_TRACT_2010_SUP <- US_TRACT_2010_JOINED[!(is.na(US_TRACT_2010_JOINED$BSH_2020_Elasticity_Units)),] %>% st_drop_geometry()
...
#_________________________LAND COVER DATABASE CONSTRUCT, getting margins of single detached housing______________________________
LandCoverNLCS <- read_stata("NANDA landcover/nanda_landcover_tract_2001-2016_02P.dta")
LandCoverNLCS <-  LandCoverNLCS[LandCoverNLCS$year_intp == 2011,] #Select only 
LandCoverNLCS <- LandCoverNLCS %>% rename(GEOID10 = tract_fips)
LandCoverNLCS <- LandCoverNLCS %>% select(GEOID10, prop_value_22, prop_value_23) #"Light" and "Medium" density development fraction-- most commonly contain single family housing


#Merging with data 

US_TRACT_2010_JOINED <- left_join(US_TRACT_2010_JOINED, LandCoverNLCS, by = c("GEOID10"))

#Constructing estimate of single detached DENSITY margin
US_TRACT_2010_JOINED_D <- US_TRACT_2010_JOINED[!is.na(US_TRACT_2010_JOINED$qyye008) & !is.na(US_TRACT_2010_JOINED$qyye009) &
                                                 !is.na(US_TRACT_2010_JOINED$qyye002) & !is.na(US_TRACT_2010_JOINED$qyye003) & !is.na(US_TRACT_2010_JOINED$qyye001) &
...
save(US_TRACT_2010_JOINED, file = "Constructed_2010_Tract.Rdata")
save(US_CBSA_2010_c, file = "CBSA_med_house_price.Rdata")
save(quantile_CBSA_houseval, file = "CBSA_quantiles.Rdata")

rm(US_TRACT_2010_JOINED, US_CBSA_2010_c, quantile_CBSA_houseval, US_CBSA_2010, LandCoverNLCS)


#_Construct ACS data for analysis 